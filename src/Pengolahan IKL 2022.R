...
eigmat <- as.matrix(eigenv22$eigenvalue[1:3])
t(eigmat)
...
kontrib <- (cj/sum(eigenv22$eigenvalue[1:3]))
kontrib
t(kontrib)
...
ggplot(dt, aes(x = var, y = kontrib)) +
  ...
  labs(title = "Kontribusi Variabel Terhadap IKL 2022", x = "Variabel", y = "Kontribusi (%)")+
  theme_minimal()+
