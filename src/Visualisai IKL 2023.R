...
# Copying k3 to k3_new
k3 <- k3

# Mengubah nilai-nilai di k3_new$cluster sesuai dengan permintaan
k3$cluster <- ifelse(k3$cluster == 1, 3, ifelse(k3$cluster == 2, 1, ifelse(k3$cluster == 3, 2, k3$cluster)))
...
#EXPORT DATA
setwd("D:/2024/Datathon/Pengolahan Data/IKL 2023/Visualisasi Hasil Analisis/")
write.csv2(datafinal, "datafinal.csv", row.names=FALSE)

petajabar = data.frame(petajabar, datafinal[,17:26])
st_write(petajabar, "jabar2023final.shp")
